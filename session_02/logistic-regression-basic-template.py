...
batchnum = int(num_train_samples / batch_size)
for i in range(num_epochs):
    c = np.concatenate((train_X, train_y[..., np.newaxis]), axis=1)
    np.random.shuffle(c)
    current_data = c[:,:-1]
    current_labels = c[:,-1:]
    for j in range(batchnum):
        batch_loss = train_function((current_data[j*batch_size:(j+1)*batch_size],
                                     current_labels[j*batch_size:(j+1)*batch_size]))
